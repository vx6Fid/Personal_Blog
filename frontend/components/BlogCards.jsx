...
function BlogCards({ blog, id }) {
  ...
  return (
    <article
      key={id}
      className="group relative p-6 sm:p-8 rounded-sm border border-borders
        hover:border-accent/40 transition-all duration-300
        hover:shadow-[0_0_20px_-6px_var(--color-glow)]"
    >
      ...
      <Link href={`/blogs/${blog.slug}`} className="block mb-3">
        <h2 className="text-xl sm:text-2xl font-semibold text-primary/90 group-hover:text-accent transition-colors duration-200 leading-tight font-display">
      ...
      {blog.tags?.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {blog.tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 text-xs font-mono rounded-sm border border-borders/40 text-secondary uppercase tracking-wide
