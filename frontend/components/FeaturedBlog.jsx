  ...
  return (
    <article
      className="group relative p-8 rounded-sm border border-accent/30
        ...
        hover:border-accent/50 hover:shadow-[0_0_30px_-8px_var(--color-glow)]"
    >
      ...
      {/* Featured badge */}
      <div className="inline-flex items-center gap-1.5 px-2 py-0.5 mb-4 text-xs font-mono rounded-sm border border-accent/40 text-accent bg-accent/10 uppercase tracking-widest">
        <span className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
        FEATURED
      </div>

      {/* Meta */}
      <div className="flex items-center gap-2 mb-3 text-xs text-secondary font-mono uppercase tracking-wider">
      ...
      <Link href={`/blogs/${blog.slug}`} className="block mb-3">
        <h2 className="text-xl sm:text-2xl font-bold text-primary group-hover:text-accent transition-colors duration-200 leading-tight font-display">
      ...
      {blog.tags?.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {blog.tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 text-xs font-mono rounded-sm border border-accent/20 text-accent/70 uppercase tracking-wide"
            >
              #{tag.toLowerCase()}
            </span>
          ))}
